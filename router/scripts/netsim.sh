...
tc qdisc del dev $IFACE root >> $LOG_PATH 2>&1

# netem settings
tc qdisc add dev $IFACE root handle 1: netem delay $DELAY $DELAY_DEVIATION loss $LOSS >> $LOG_PATH 2>&1
...
tc qdisc add dev $IFACE parent 1: handle 2: tbf rate $RATE burst $BURST limit $LIMIT >> $LOG_PATH 2>&1
fi

# print results	
STATUS_NETEM=$(tc qdisc show dev $IFACE root | awk -F "1000 " '{print $2}')
STATUS_TBF=$(tc qdisc show dev $IFACE | awk -F "rate|burst" '{print $2}')
echo "$HOST: $STATUS_NETEM, $STATUS_TBF."
