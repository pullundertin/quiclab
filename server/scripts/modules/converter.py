...
def process_all_input_files(tcpprobe_directory, selected_columns):
    for filename in os.listdir(tcpprobe_directory):
        if filename.endswith(".log"):
            input_file = os.path.join(tcpprobe_directory, filename)
            output_file = os.path.join(
                tcpprobe_directory, f"{os.path.splitext(filename)[0]}.csv")
